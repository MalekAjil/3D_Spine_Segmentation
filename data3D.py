...
def create_train_data():
    ...
    for dirr in sorted(os.listdir(train_data_path)):
        ...
        for image_name in images:
            ...
            # im = Image.open(os.path.join(dirr, image_name))
            # new_size = (400, 400)
    ...
    for dirr in sorted(os.listdir(train_data_path)):
        ...
        for mask_name in images:
            ...
            # im = Image.open(os.path.join(dirr, mask_name))
            # new_size = (400, 400)
            # img_mask = im.resize(new_size)
...
def create_test_data():
    ...
    for dirr in sorted(os.listdir(test_data_path)):
        ...
        count = total
        for image_name in images:
            img = imread(os.path.join(dirr, image_name))
            img = img.astype(np.uint8)
            ...
            # print("imgs[{0}][{1}]=".format(i, j) + image_name)
            ...
            if j % (image_depth - 1) == 0:
                if i + 1 <= image_depth + 1:
                    # print("i={0}, j={1}".format(i, j))
                    imgs[i + 1][0] = img
                    # print("imgs[{0}][0]=".format(i+1) + image_name)

            if j % image_depth == 0:
                if i + 1 <= image_depth + 1:
                    imgs[i + 1][1] = img
                    # print("imgs[{0}][1]=".format(i+1) + image_name)
                j = 2
                i += 1
                if (i % 100) == 0:
